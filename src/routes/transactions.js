...
module.exports = (app) => {
  ...
  router.param('id', async (req, res, next) => {
    const result = await app.services.transaction.validateUser(req.user.id, { tr_id: req.params.id });
  ...
  router.get('/', async (req, res, next) => {
    try {
      const result = await app.services.transaction.validateUser(req.user.id);
  ...
  router.get('/:id', async (req, res, next) => {
    try {
      const result = await app.services.transaction.find({ tr_id: req.params.id });
      res.status(200).json(result);
    } catch (err) {
      next(err);
    }
